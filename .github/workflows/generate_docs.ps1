...
function New-TableOfContents {
    ...
    foreach ($category in $categories) {
        ...
        foreach ($rule in ($category.Group | Sort-Object -Property RuleId )) {
            ...
            [void]$StringBuilder.AppendLine("| $ruleId | $($Rule.FriendlyName -replace "\|", "&#124;") | $(if ($Rule.IsIgnorable -ieq "true" ) { "Yes" } else { " " }) | $($Rule.Description -replace "\|", "&#124;") | $(if ([string]::IsNullOrWhiteSpace($Rule.Example)) { " " } else { "Yes" }) |")
        }
    }
